...
module "ecr" {
  ...
  repository_lambda_read_access_arns = [module.trigger_glue_job.lambda_function_arn]
}


# Lambda function using terraform-aws-modules/lambda/aws module
module "trigger_glue_job" {
  source  = "terraform-aws-modules/lambda/aws"
  version = "~> 7.20"

  function_name = "${local.project_name}-trigger-glue-job"
  description   = "Lambda function to trigger Glue job when a file is uploaded to S3"
  ...
  environment_variables = {
    ...
    STATE_MACHINE_ARN = aws_sfn_state_machine.etl_state_machine.arn
  }
...
resource "aws_s3_bucket_notification" "bucket_notification" {
  ...
  lambda_function {
    lambda_function_arn = module.trigger_glue_job.lambda_function_arn
...
resource "aws_lambda_permission" "allow_bucket" {
  ...
  function_name = module.trigger_glue_job.lambda_function_arn
