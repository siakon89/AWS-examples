...
from pyspark.sql.types import StructType, StructField, StringType, IntegerType, DoubleType, TimestampType, DateType, LongType, BooleanType
...
def get_column_names_from_s3_or_local(path):
    ...
    if path.startswith('s3://'):
        ...
        # Use boto3 to read the first line from S3
        s3 = boto3.client('s3')
        # Get just the first chunk of the file to read the header
...
def process_csv_to_parquet():
    ...
    spark_df = spark.read.option("header", "true").option("inferSchema", "true").csv(input_path)
    ...
    if len(field_names) == len(column_names):
        ...
        mapping = []
        for i, (field_name, desired_name) in enumerate(zip(field_names, column_names)):
            ...
            if desired_name in column_types:
                ...
                print(f"Mapping {field_name} ({glue_type}) -> {desired_name} ({glue_type})")
            else:
        ...
        raise Exception("Column names do not match between Spark DataFrame and dynamic frame.")
    ...
    print(f"Successfully converted {record_count} records from CSV to Parquet at {output_path}")
