...
resource "aws_iam_role" "glue_crawler" {
  name = "${local.name}-demo-crawler-role"
...
resource "aws_iam_policy" "glue_crawler" {
  name        = "${local.name}-demo-crawler-policy"
  path        = "/"
  description = "Policy for Glue crawler to access demo data"
    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        Resource = [
          "arn:aws:s3:::${module.athena_data_bucket.s3_bucket_id}",
          "arn:aws:s3:::${module.athena_data_bucket.s3_bucket_id}/*"
        ]
      },
      {
